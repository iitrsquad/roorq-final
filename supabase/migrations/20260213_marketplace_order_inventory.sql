-- Migration: Marketplace order safety + inventory finalization
-- Date: 2026-02-13

BEGIN;

-- Ensure marketplace order function validates vendor assignment
...
BEGIN
  ...
  LOOP
    ...
    SELECT
      ...
      stock_quantity,
      vendor_id
    INTO v_price, v_is_active, v_approval_status, v_stock, v_vendor_id
    ...
    IF v_vendor_id IS NULL THEN
      RAISE EXCEPTION 'Product missing vendor assignment: %', v_item.product_id USING ERRCODE = 'P0001';
    END IF;
...
-- Inventory + payment finalization on vendor order status change
CREATE OR REPLACE FUNCTION public.handle_vendor_order_status_change()
RETURNS TRIGGER AS $$
DECLARE
  item RECORD;
BEGIN
  ...
  -- If delivered, finalize inventory and mark parent paid if all delivered
  IF NEW.status = 'delivered' AND OLD.status IS DISTINCT FROM 'delivered' THEN
    FOR item IN
      SELECT product_id, quantity
      FROM public.vendor_order_items
      WHERE vendor_order_id = NEW.id
    LOOP
  ...
  -- If cancelled/returned/refunded before delivery, release reserved stock
  IF NEW.status IN ('cancelled', 'returned', 'refunded') AND OLD.status IS DISTINCT FROM 'delivered' THEN
    FOR item IN
      SELECT product_id, quantity
      FROM public.vendor_order_items
      WHERE vendor_order_id = NEW.id
    LOOP
      PERFORM release_inventory(item.product_id, item.quantity);
    END LOOP;
  END IF;

  -- If returned after delivery, restock physical inventory
...
DROP TRIGGER IF EXISTS trigger_handle_vendor_order_status_change ON public.vendor_orders;
CREATE TRIGGER trigger_handle_vendor_order_status_change
  AFTER UPDATE OF status ON public.vendor_orders
  FOR EACH ROW
  EXECUTE FUNCTION public.handle_vendor_order_status_change();
